...
public class CriticalHits extends Module {
    ...
    public void input(Input input) {
        ...
        if (this.freeze > 0) {
            state = new PlayerInput(
                false, state.backward(),
                state.left(), state.right(), state.jump(), state.sneak(),
                false
            );

            float forward = state.backward() ? -1.0F : 0.0F;
            API.move(input, forward, input.getMovementInput().x);
